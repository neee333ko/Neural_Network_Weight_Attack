...
# 不正则化衰减参数
class CIFAR10NetBeta(nn.Module):
    def __init__(self, channels=256, **kwargs):
        super().__init__()
        ...
        for i in range(2):
            for j in range(3):
                ...
                conv2d = quan_Conv2d(in_channels, channels, kernel_size=3,  padding=1, bias=False)
                bn = layer.BatchNorm2d(channels)
                node = neuron.ParametricLIFNode(**deepcopy(kwargs))
                node.pre_layer = conv2d
        ...
        liner1 = quan_Linear(channels * 8 * 8, 2048)
        node1 = neuron.ParametricLIFNode(**deepcopy(kwargs))
        node1.pre_layer = liner1
        
        liner2 = quan_Linear(2048, 100)
        node2 = neuron.ParametricLIFNode(**deepcopy(kwargs))
        node2.pre_layer = liner2
...
def cifar10netbeta(pretrained=False, **kwargs):
    """Constructs a ResNet-152 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = CIFAR10NetBeta()
    return model
